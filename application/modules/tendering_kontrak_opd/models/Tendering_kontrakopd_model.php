...
class Tendering_kontrakopd_model extends CI_Model {
    ...
    public function lelang_resume($tahun, $satker){
        ...
        $this->pg_db->join("kontrak c", "b.lls_id = c.lls_id");
        if ($tahun != 'all') {
            $this->pg_db->where("b.tahun", $tahun+0);
        }
        if ($satker != 'all') {
            $this->pg_db->where("b.agc_id", $satker);
        }
        $data = $this->pg_db->get();
        return $data;
    }
    ...
    public function total_kontrak_opd($tahun, $satker){
        $this->pg_db->select("
            COUNT(a.lls_id) AS jml_kontrak, 
            SUM(b.pkt_hps) AS jml_hps,
            SUM(b.harga_terkoreksi) AS nilai_penawaran,
    ...
    public function paket_kontrak($tahun, $satker){
        $this->pg_db->select("
            ...
            b.pkt_hps,
            b.harga_terkoreksi,
            CASE WHEN a.lls_versi_lelang > 1::numeric THEN 'Lelang Ulang'::text ELSE ' '::text END AS status,    
            c.kontrak_no,
        ...
        $this->pg_db->join("sip.narno_menang b", "a.lls_id = b.lls_id");
        $this->pg_db->join("kontrak c", "b.lls_id = c.lls_id");
        $this->pg_db->join("sip.narno_rup d", "a.lls_id = d.lls_id");
