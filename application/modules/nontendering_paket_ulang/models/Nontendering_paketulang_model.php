...
class Nontendering_paketulang_model extends CI_Model {
    ...
    public function total_rekap_retendering($tahun){
        $this->pg_db->select("
            COUNT(a.pkt_nama) AS jml_pkt_ulang,
            SUM(a.pkt_pagu) AS jml_pagu_ulang
        ");
    ...
    public function data_retendering($tahun){
        $this->pg_db->select("
            ...
            (
                CASE
                    WHEN b.lls_versi_lelang = 2 THEN b.lls_ditutup_karena
                    WHEN b.lls_versi_lelang = 3 THEN b.lls_ditutup_karena
                ELSE b.lls_diulang_karena
                END
            ) AS alasan
        ");
        $this->pg_db->from("narno_pl_semua a");
