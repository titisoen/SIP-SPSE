...
class Rekanan_info_controller extends CI_Controller {
	...
	public function rekap_rekanan($tahun){
		$repo_id = $this->model->get_repo();

		$result	= $this->model->rekap_rekanan($tahun, $repo_id);
		$data 	= array();
		foreach ($result->result() as $rows) {
			$data 	= array(
							"total_rekanan"					=> number_format($rows->rekanan_semua),
							"total_verifikasi"				=> number_format($rows->rekanan_verifikasi),
							"total_roaming"					=> number_format($rows->rekanan_roaming),
						);
		}
	...
	public function rekanan_roaming($tahun){
		$repo_id = $this->model->get_repo();
		...
		$result_data		= $this->model->rekanan_roaming($tahun, $repo_id);
		foreach ($result_data->result() as $rows_data) {
			$data["baris_data"][] = array(
											"no"							=> $no++,
											"rkn_nama"		 				=> strtoupper($rows_data->rkn_nama),
